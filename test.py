...
class TestECCArithmetic(unittest.TestCase):
    ...
    def test_all(self):
        for i in range(8):
            ...
            self.assertEqual(
                multiply(multiply(G, x), y)[0], multiply(multiply(G, y), x)[0]
            )
            self.assertEqual(
            ...
            self.assertEqual(
                multiply(multiply(G, hx), hy)[0], multiply(multiply(G, hy), hx)[0]
            )
            self.assertEqual(
...
class TestRawSignRecover(unittest.TestCase):
    ...
    def test_all(self):
        for i in range(20):
            ...
            self.assertEqual(
                ecdsa_raw_recover("35" * 32, s), decode_pubkey(privtopub(k))
            )
...
#  for i in range(10):
#  msg = ''.join(
#  [random.choice(alphabet) for i in range(random.randrange(20, 200))]
#  )
...
class TestSerialize(unittest.TestCase):
    def test_serialize(self):
        ...
        self.assertEqual(serialize(deserialize(tx)), tx, "Serialize roundtrip failed")
...
class TestDeterministicGenerate(unittest.TestCase):
    ...
    def test_all(self):
        ...
        for i, ti in enumerate(test_vectors):
            ...
            self.assertEqual(
                ti, mine, "Test vector does not match. Details:\n%s\n%s" % (ti, mine)
            )
...
class TestBIP0032(unittest.TestCase):
        ...
        for tv in test_vectors:
            ...
            self.assertEqual(
                ...
                % (
                    tv[0],
                    [
                        x.encode("hex") if isinstance(x, str) else x
                        for x in bip32_deserialize(left)
                    ],
                    [
                        x.encode("hex") if isinstance(x, str) else x
                        for x in bip32_deserialize(right)
                    ],
                ),
            )
    ...
    def test_all_testnet(self):
        ...
        mk = bip32_master_key(
            safe_from_hex("000102030405060708090a0b0c0d0e0f"),
            TESTNET_PRIVATE,
        )
    ...
    def test_extra(self):
        ...
        assert (
            bip32_privtopub(bip32_ckd(master, "0"))
        ...
        assert (
            bip32_ckd(
                bip32_ckd(
                    bip32_ckd(
                        bip32_ckd(bip32_ckd(master, 44 + 2 ** 31), 2 ** 31), 2 ** 31
                    ),
                    0,
                ),
                0,
...
class TestRipeMD160PythonBackup(unittest.TestCase):
    ...
    def test_all(self):
        ...
        for i, s in enumerate(strvec):
            ...
            self.assertEqual(
                bytes_to_hex_string(bin_hash160(from_string_to_bytes(s))),
                hash160target[i],
            )
...
class TestConversions(unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        cls.privkey_bin = b"\xe9\x87=y\xc6\xd8}\xc0\xfbjWxc3\x89\xf4E2\x130=\xa6\x1f \xbdg\xfc#:\xa32b"

        cls.pubkey_hex = "04588d202afcc1ee4ab5254c7847ec25b9a135bbda0f2bc69ee1a714749fd77dc9f88ff2a00d7e752d44cbe16e1ebcf0890b76ec7c78886109dee76ccfc8445424"
        cls.pubkey_bin = b"\x04X\x8d *\xfc\xc1\xeeJ\xb5%LxG\xec%\xb9\xa15\xbb\xda\x0f+\xc6\x9e\xe1\xa7\x14t\x9f\xd7}\xc9\xf8\x8f\xf2\xa0\r~u-D\xcb\xe1n\x1e\xbc\xf0\x89\x0bv\xec|x\x88a\t\xde\xe7l\xcf\xc8DT$"
    ...
    def test_changebase(self):
        self.assertEqual(
            self.pubkey_bin,
            changebase(self.pubkey_hex, 16, 256, minlen=len(self.pubkey_bin)),
        )

        self.assertEqual(
            self.pubkey_hex,
            changebase(self.pubkey_bin, 256, 16, minlen=len(self.pubkey_hex)),
        )
        ...
        self.assertEqual(
            self.privkey_hex,
            changebase(self.privkey_bin, 256, 16, minlen=len(self.privkey_hex)),
        )
